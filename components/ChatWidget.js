...
export default function ChatWidget({onClose}) {
  ...
  <TouchableOpacity onPress={() => copyToClipboard(msg.text)} style={styles.copyIcon}>
    <Feather name="copy" size={18} color="#fff" />
            ...
            {msg.items && msg.items.map((item, i) => (
              <TouchableOpacity
                key={i}
                onPress={() => handleImageSelect(item.images)}
                style={[
                  styles.itemContainer,
                  selectedImage === item.images && { borderColor: 'green', borderWidth: 2 }
                ]}
              >
                <Image
      ...
      <TouchableOpacity onPress={()=>handleTryOn(selectedImage)} style={styles.tryOnButton}>
...
sendButton: {
  backgroundColor: '#5c2c89',       // elegant deep purple
  borderRadius: 25,                 // more rounded
  paddingVertical: 12,              // vertical padding for better tap target
